...
def travels_service(request_data: RequestModel):
    ...
    try: 
        ...
        if request_data.channel is None or request_data.channel == 'PHONE':
        ...
        if request_data.channel is not None and request_data.channel == 'WEB':

            leaving_travel_options = [f"1. Viaje en Aerobot con salida a las {leaving_travel_hours[0]} de la mañana con un costo de {leaving_travel_prices[0]} pesos.",
                                      f"2. Viaje con Aeromex a las {leaving_travel_hours[1]} de la mañana con un costo de {leaving_travel_prices[1]} pesos.", 
                                      f"3. Viaje con Aviones Nacionales a las {leaving_travel_hours[2]} de la mañana con un costo de {leaving_travel_prices[2]} pesos."] 

            # Returning trip text
            if request_data.trip_type == "redondo" and request_data.returning_date != "":
                
                returning_travel_options = [f"1. Viaje en Aviones Nacionales con salida a las {returning_travel_hours[0]} de la mañana con un costo de {returning_travel_prices[0]} pesos.",
                                            f"2. Viaje con Aerobot a las {returning_travel_hours[1]} de la mañana con un costo de {returning_travel_prices[1]} pesos.",
                                            f"3. Viaje con Aeromex a las {returning_travel_hours[2]} de la mañana con un costo de {returning_travel_prices[2]} pesos."]

            else:
                returning_travel_options = []
            ...
            response = {
                    ...
                    "leaving_travel":{
                        ...
                        "prices": leaving_travel_prices,
                        "dates": leaving_travel_dates,
                        "options":leaving_travel_options
                    },
                    "returning_travel": {
                        "sales_text": returning_travel_text,
                        "prices": returning_travel_prices,
                        "dates": returning_travel_dates,
                        "options":returning_travel_options
                    }
            }
